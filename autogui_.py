...
class AutoGui:
    ...
    def _parse_result(self, sql):
        ...
        if parse_dict:
            ...
            second = sql.split('from')[-1].split('where')[0]
            third = sql.split('by')[-1]
            save_path = os.path.join('./parses', 'result.json')
    ...
    def run(self, sql):
        ...
        for i in range(page_num):
            ...
            for date, cont, pinyin in zip(result_clean[::3], result_clean[1::3], result_clean[2::3]):
        ...
        save_path = os.path.join('./results', 'result.json')
        with open(save_path, 'w', encoding='utf-8') as js:
